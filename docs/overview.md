...
Uniform DCA is the benchmark because it is realistic, transparent, and already strong. It spreads purchases evenly through time, avoids discretionary overreaction, and gives the project a serious baseline that many investors would actually consider using. The first baseline-foundation model already demonstrated why this comparison was meaningful: even a simple interpretable on-chain rule reached a 54.36% score with a 68.13% win rate, which was enough to show that the project could improve on uniform DCA without abandoning transparency. Any later dynamic model therefore had to justify its extra complexity by improving on a rule that was not only simple and defensible, but already measurably competitive.
...
This project shows that a dynamic, interpretable, long-only Bitcoin accumulation rule can improve meaningfully on uniform DCA under the same rolling-budget constraints.
